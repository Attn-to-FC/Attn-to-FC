...
def gendescr_2inp(model, data, comstok, comlen, batchsize, config, w=1):
...
def gendescr_3inp(model, data, comstok, comlen, batchsize, config, w=1):
...
def gendescr_4inp(model, data, comstok, comlen, batchsize, config, w=1):
...
def gendescr_5inp(model, data, comstok, comlen, batchsize, config, w=1):
...
def gendescr_graphast(model, data, comstok, comlen, batchsize, config, w=1):
...
def gendescr_pathast(model, data, comstok, comlen, batchsize, config, w=1):
...
def gendescr_threed(model, data, comstok, comlen, batchsize, config, w=1):
...
if __name__ == '__main__':
    ...
    stk = comstok.w2i['<s>']
    comstart[0] = stk
    outfn = outdir+"/predictions/predict-{}-beamwidth-{}.txt".format(outfile.split('.')[0], beam_width)
    outf = open(outfn, 'w')
    ...
    for c, fid_set in enumerate(batch_sets):
        ...
        if config['batch_maker'] == 'datsonly':
            batch_results = gendescr_2inp(model, batch, comstok, comlen, batchsize, config, beam_width)
        elif config['batch_maker'] == 'ast':
            batch_results = gendescr_3inp(model, batch, comstok, comlen, batchsize, config, beam_width)
        elif config['batch_maker'] == 'ast_threed':
            batch_results = gendescr_4inp(model, batch, comstok, comlen, batchsize, config, beam_width)
        elif config['batch_maker'] == 'threed':
            batch_results = gendescr_threed(model, batch, comstok, comlen, batchsize, config, beam_width)
        elif config['batch_maker'] == 'graphast':
            if testval == 'test':
                batch = batch[0]
            batch_results = gendescr_graphast(model, batch, comstok, comlen, batchsize, config, beam_width)
        elif config['batch_maker'] == 'graphast_threed':
            if testval == 'test':
                batch = batch[0]
            batch_results = gendescr_5inp(model, batch, comstok, comlen, batchsize, config, beam_width)
        elif config['batch_maker'] == 'pathast_threed':
            if testval == 'test':
                batch = batch[0]
            batch_results = gendescr_pathast(model, batch, comstok, comlen, batchsize, config, beam_width)
        else:
